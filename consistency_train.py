...
parser.add_argument("--conf",
                    ...
                    default="./conf/consistency_train.yaml")
...
if __name__ == "__main__":
    with open(args.conf, 'r') as f:
        ...
        # data loader
        labeled_loader, unlabeled_loader, val_loader = initialize.double_data_loader(train_cfg.label_data, train_cfg.unlabeled_data, \
        ...
        for epoch in range(train_cfg.start_epoch, train_cfg.end_epoch):
            ...
            for i in tqdm(range(train_iter_len)):
                try:
                    label_inputs = labeled_iter.next()
                except:
                    labeled_iter = iter(labeled_loader)
                    label_inputs = labeled_iter.next()
                try:
                    unlabel_inputs = unlabeled_iter.next()
                except:
                    unlabeled_iter = iter(unlabeled_loader)
                    unlabel_inputs = unlabeled_iter.next()

                for (l_key, l_ipt), (u_key, u_ipt) in zip(label_inputs.items(), unlabel_inputs.items()):
                    label_inputs[l_key] = l_ipt.to(device)
                    unlabel_inputs[u_key] = u_ipt.to(device)
                
                with torch.cuda.amp.autocast(enabled=True):
                    total_loss, losses = loss.compute_semi_loss(label_inputs, unlabel_inputs, model, train_cfg)
